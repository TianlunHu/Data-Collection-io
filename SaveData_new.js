...
downloadButton.addEventListener('click', () => {
    ...
    const anacc = new Blob(AnA, {
        type: "text/plain;charset=utf-8"
    });
    saveAs(anacc, "AngularAcceleration  " + T + ".txt");
...
function StartSensor() {
    ...
    function MotionHandler(acceleration, gyroscope, orientation, AV, AnA, OV) {
        // ---------- Orientator ----------- //
        ...
        // ---------- Angular Accelerater ----------- //
        let info_a, An_xyz = "An_X An_Y An_Z";

        info_a = An_xyz.replace("An_X", gyroscope.x && gyroscope.x.toFixed(3));
        info_a = info_a.replace("An_Y", gyroscope.y && gyroscope.y.toFixed(3));
        info_a = info_a.replace("An_Z", gyroscope.z && gyroscope.z.toFixed(3));
        AnA.push(info_a + ' ');
        document.getElementById('moAnAccel').innerHTML = info_a;
    ...
    if ('LinearAccelerationSensor' in window  && 'DeviceOrientationEvent' in window && 'AbsoluteOrientationSensor' in window && 'Gyroscope' in window) {
        ...
        gyroLowPass = new LowPassFilterData(gyroHighPass, 0.8);
        
        accelerometer.onreading = () => {
            accHighPass.update(accelerometer);
            accLowPass.update(accHighPass);
            gyroHighPass.update(gyroscope);
            gyroLowPass.update(gyroHighPass);
            MotionHandler(accLowPass, gyroLowPass, orientator, AccVec, AnAccVec, OriVec);
...
//            gyroHighPass.update(gyroscope);
//            gyroLowPass.update(gyroHighPass);
//            MotionHandler(gyroLowPass, orientator, AccVec, OriVec);
//            let current = (accelerometer.timestamp / 1000).toFixed(3);
//            document.getElementById("timeStamp").innerHTML = current;
//            TsVec.push(current + ' ');
//            
